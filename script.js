...
// raw cursor position in pixels, used only by the cursor comet trail
...
const comets = [];
const particles = [];
const cursorTrail = []; // history of recent cursor points for the comet trail
...
function drawBackground() {
  ...
  g.addColorStop(0, config.background[0]);
  g.addColorStop(1, config.background[1]);
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, width, height);
}

function animate(time) {
  const delta = prefersReducedMotion ? 0 : Math.min(time - lastTime, 40);
  lastTime = time;

  driftT += delta;
  // idle autonomous sway, blended with pointer input so it never feels static
  const idleX = Math.sin(driftT * 0.00012) * 0.15;
  const idleY = Math.cos(driftT * 0.00009) * 0.1;

  pointer.x += (pointer.targetX + idleX - pointer.x) * 0.04;
  pointer.y += (pointer.targetY + idleY - pointer.y) * 0.04;
  ...
  const { cx, cy } = planets[0] ? planets[0].center() : { cx: width / 2, cy: height / 2 };
  const beltOrbitX = width * 0.62;
  const beltOrbitY = beltOrbitX * 0.62;
  asteroids.forEach(a => {
    a.update(delta);
    a.draw(cx, cy, beltOrbitX, beltOrbitY);
  });
